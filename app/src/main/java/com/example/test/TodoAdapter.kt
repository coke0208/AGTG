...
class TodoAdapter(var Todos: List<Todo>,
    ...
    inner class ToDoViewHolder(private val itemBinding: ItemViewBinding): RecyclerView.ViewHolder(itemBinding.root){
        fun bind(data:Todo) {
    ...
    fun setColdData(coldData: List<Todo>){
        Todos = coldData
        notifyDataSetChanged()
    }

    fun setFrozenData(frozenData: List<Todo>){
        Todos = frozenData
        notifyDataSetChanged()
    }

    fun setRoomData(roomData: List<Todo>){
        Todos = roomData
        notifyDataSetChanged()
    }
}