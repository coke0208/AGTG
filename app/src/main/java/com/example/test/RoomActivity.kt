...
class RoomActivity: Fragment() {
    ...
    override fun onCreateView(
        ...
        // 3. adapter 설정
        var roomList = viewModel.roomList.value
        adapter = TodoAdapter(
            roomList?: emptyList<Todo>(),
            onClickDeleteButton={
        ...
        binding.roomlist.layoutManager = LinearLayoutManager(activity)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.todoList.observe(viewLifecycleOwner, Observer{
            binding.roomlist.post(Runnable { adapter.setRoomData(it.filter { x->x.isDone }) })
        })
    }
}